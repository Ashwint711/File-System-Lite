...
struct DecodeResult Decode(FILE *vdfile, unsigned int count)
{
	struct DecodeResult result;
	result.bits_to_skip = 0;
	result.bytes_to_skip = 0;

	int file_pointer_pos = ftell(vdfile);
...
/*... FETCHING count of Bits in which Count/Length is stored. ...*/
	...
	{
		// INITIALIZATION	
		bits_visited = (result.bits_to_skip % 8);
		rem_bits = (8 - (result.bits_to_skip % 8));
		cur_bits = 2;
		bit_seq_len = 0;
		
		fp_pos = (file_pointer_pos + (result.bits_to_skip) / 8);
	...
	//	fseek(vdfile, count, SEEK_SET);
		...
		unsigned int total_bits = val + bit_seq_len;
		total_bits += result.bits_to_skip; //total bits from first Byte
	...
	/*.. Moving File Pointer to Last Byte of Sequence. ..*/
		fp_pos = ((file_pointer_pos * 8) + total_bits) % 8 == 0 ? (((file_pointer_pos * 8) + total_bits) / 8) - 1 : ((file_pointer_pos * 8) + total_bits) / 8;
		fseek(vdfile, fp_pos, SEEK_SET);
		...
		*(res) &= (0xff >> ((!!(val % 8)) * (8 - (val % 8))));//Don't shift when remainder is 0.
		...
		result.bits_to_skip = total_bits;
